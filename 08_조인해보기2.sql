...
SELECT
    *
FROM info i
INNER JOIN auth a  --JOIN 만 써도 가능
...
SELECT
    *
FROM info i LEFT JOIN auth a -- 기준: info (i)  LEFT OUTER JOIN
ON i.auth_id = a.auth_id;
...
SELECT
    *
FROM info i RIGHT JOIN auth a -- 기준: auth (a)  RIGHT OUTER JOIN 
ON i.auth_id = a.auth_id;
...
-- 실제로는 거의 사용하지 않는다.   행6 * 행5 = 30개 의 행 정처기나옴.
SELECT * FROM info
CROSS JOIN auth
ORDER BY id ASC;


-- 여러개 테이블 조인 -> 키 값만 찾아서 구문을 연결해서 쓰면 된다.
SELECT
    *
FROM employees e
...
WHERE a.job = 'scientist'; -- auth를 기준으로 잡은이유 a.job=scientist먼저 걸러주기때문
