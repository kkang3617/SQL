...
CREATE TABLE emps_it AS (SELECT * FROM employees WHERE 1 = 2); --1=2 구조만 가져옴.
SELECT * FROM emps_it;
...
기존의 데이터는 email, phone, salary, comm_pct, mana_id, dept_id을 
업데이트 하도록 처리
새로 유입돈 데이터는 그대로 추가.
*/
...
WHEN NOT MATCHED THEN
    INSERT VALUES   -- a에 b의 값을 전부를 넣어줘
    (b.employee_id, b.first_name, b.last_name,
    b.email, b.phone_number, b.hire_date, b.job_id,
    b.salary, b.commission_pct, b.manager_id, b.department_id);
...
WHERE department_name IN ('개발' , '회계부', '재정', '인사');
...
MERGE INTO depts a
    ...
    ON -- 병합시킬 데이터의 연결 조건
        (a.department_id = b.department_id)  --매칭조건 
WHEN MATCHED THEN
    UPDATE SET  --매칭조건 a,b 둘다 일치하는 경우에 업데이트
...
MERGE INTO jobs_it a
    ...
    ON
        (a.job_id = b.job_id)      -- 매칭조건만 작성하면 된다.
WHEN MATCHED THEN
    UPDATE SET
