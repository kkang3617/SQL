...
CREATE VIEW view_emp AS (
    SELECT
        employee_id,
        first_name || ' ' || last_name AS name,  -- 뷰의 컬럼 이름은 함수 같은
        job_id,                                 -- 가상표현식이면 안된다. AS사용!
        salary
    FROM employees
    WHERE department_id = 60
);
...
/*
VIEW에 INSERT가 일어나는 경우 실제 테이블에도 반영이된다.
그래서 VIEW의 INSERT, UPDATE, DELETE 는 많은 제약사항이 따른다.
...
VALUES(300, 'test', 'test', 10000); -- 에러

-- 원본 테이블의 null을 허용하지 않는 컬럼 때문에 들어갈 수 없습니다.
INSERT INTO view_emp
(employee_id, job_id, salary)
VALUES(300, 'test', 10000); -- 에러
