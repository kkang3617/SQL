...
SELECT TO_CHAR(sysdate, 'YYYY-MM-DD DY DAY PM HH:MI:SS') FROM dual;
SELECT TO_CHAR(sysdate, 'YY-MM-DD HH24:MI:ss') FROM dual; --년 두자리, 24시간표현
...
SELECT TO_CHAR(20000.29, '99999.9') FROM dual; --반올림도 해줌.
SELECT TO_CHAR(20000, '99,999') FROM dual;

SELECT TO_CHAR(salary, '$99,999') AS salary
FROM employees;
SELECT TO_CHAR(salary, 'L99,999') AS salary  --사용하는 국가 통화적용
FROM employees;

-- 문자를 숫자로 TO_NUMBER(값, 형식)
SELECT '2000' + 2000 FROM dual; -- 자동 형 변환 (문자->숫자)
SELECT TO_number('2000') + 2000 FROM dual; -- 명시적 형 변환
SELECT '$3,300' + 2000 FROM dual; -- 이렇게 하면 에러남!
SELECT TO_NUMBER('$3,300', '$9,999') + 2000 FROM dual; --9로 숫자의 위치를 알려준다.
...
SELECT
    ...
    AS real_salary
    --commission_pct 가 널인 아닌경우 수식적용, 널인경우 그냥salary
FROM employees;
...
FROM employees
WHERE (sysdate - hire_date) / 365 >= 17  -- 여기에 근속년수 쓰면 안됨!
ORDER BY 근속년수 DESC;  -- 이유? FROM -> WHERE -> SELECT ->ORDER 순으로 실행하기 때문
...
SELECT  --잘못된 예시
    ...
    employee_id,
    first_name,
    DECODE(   --DECODE문은 범위를 지정하는 값을 명시할 수 없다. 결과 안나옴
        salary, --기준                                 아래 코드 답임
        salary >=4000 , 'A',
...
SELECT  --옳은 예시 DECODE사용
    salary,
    employee_id,
    first_name,
    DECODE(   
        TRUNC(salary / 1000), --기준  소수점을 절사
        0, 'E',
        1, 'D',
        2, 'C',
        3, 'B',
...
SELECT  -- 옳은 예시 CASE WHEN THEN END 사용
    salary,
    employee_id,
    first_name,
    (CASE   
