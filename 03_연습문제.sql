...
FROM employees
WHERE department_id IN '50';  --위처럼 =50 도 가능

--5. 모든 사원의 이름, 급여 그리고 300달러 인상된 급여를 출력하세요.
SELECT first_name, salary, (salary+300) AS pay
FROM employees;
...
FROM employees
WHERE job_id LIKE '%MAN'
ORDER BY job_id ASC;   -- ASC 작성하지 않아도 동작. ORDER BY 기본값 ASC
